...
# Now with extra empty wells
QC <- as.data.frame(read.csv("Platy/6th_approach/Data/QC/metadata_withempty.xlsx"))
...
plot(log10(colSums(raw.data[!grepl("ERCC-", rownames(raw.data)),])), pch = 16, col = ifelse(as.numeric(QC$nr.of.cells) == 1 & is.na(QC$note), "goldenrod", ifelse(as.numeric(QC$nr.of.cells) > 1 & is.na(QC$note), "darkgreen", ifelse(as.numeric(QC$nr.of.cells) == 0 & is.na(QC$note), "darkblue", "black"))))
